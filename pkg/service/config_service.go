...
func (s *ConfigService) CheckToolsInstallation() (*ToolStatus, error) {
	...
	if runtime.GOOS == "windows" {
		...
		status.Gogo = s.checkUnixToolExists("gogo")
		status.Afrog = s.checkUnixToolExists("afrog")   // 新增
		status.Nuclei = s.checkUnixToolExists("nuclei") // 新增
	}

	logging.Info("工具安装状态检查完成: Nmap=%v, Ffuf=%v, Subfinder=%v, HttpX=%v, Fscan=%v, Gogo=%v,Afrog=%v, Nuclei=%v",
		status.Nmap, status.Ffuf, status.Subfinder, status.HttpX, status.Fscan, status.Gogo, status.Afrog, status.Nuclei)
...
func (s *ConfigService) GetToolConfigForTask(taskType string) (interface{}, error) {
	...
	case "gogo":
		if !config.GogoConfig.Enabled {
			return nil, fmt.Errorf("Gogo工具未启用")
		}
		return config.GogoConfig, nil
	case "afrog":
