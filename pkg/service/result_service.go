...
// GetResultsByTypeWithPagination 根据类型获取扫描结果列表（带分页）
func (s *ResultService) GetResultsByTypeWithPagination(resultType string, page, pageSize int) ([]*models.Result, int64, error) {
	if resultType == "" {
		return nil, 0, errors.New("无效的类型")
	}
	...
	logging.Info("正在通过 Service 分页获取类型为 %s 的扫描结果，页码: %d，每页数量: %d", resultType, page, pageSize)
	results, total, err := s.resultDAO.GetResultsByTypeWithPagination(resultType, page, pageSize)
	if err != nil {
		logging.Error("通过 Service 分页获取类型为 %s 的扫描结果失败: %v", resultType, err)
		return nil, 0, err
	}

	logging.Info("通过 Service 成功分页获取类型为 %s 的扫描结果，当前页: %d，共 %d 个", resultType, page, len(results))
