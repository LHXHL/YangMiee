...
func (dao *ResultDAO) GetResultsByTypeWithPagination(resultType string, page, pageSize int) ([]*models.Result, int64, error) {
	logging.Info("正在分页获取类型为 %s 的扫描结果，页码: %d，每页数量: %d", resultType, page, pageSize)

	// 计算总数
	total, err := dao.collection.CountDocuments(context.Background(), bson.M{"type": resultType})
	if err != nil {
		logging.Error("计算结果总数失败: %v", err)
		return nil, 0, err
	}
	...
	// 执行查询
	cursor, err := dao.collection.Find(context.Background(), bson.M{"type": resultType}, options)
	if err != nil {
		logging.Error("获取类型为 %s 的扫描结果失败: %v", resultType, err)
		return nil, 0, err
	}
	...
	logging.Info("成功获取类型为 %s 的扫描结果，共 %d 个，总数 %d", resultType, len(results), total)
